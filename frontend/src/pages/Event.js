...
// 종료된 이벤트 조회
  ...
  const endButtonClick = () => {
    ...
    return (
        <Container>
            <Events>
            <button className={`eventbutton ${isIngEventButtonClicked ? "active" : ""}`} onClick={ingButtonClick}>진행중인 이벤트</button>
            <button className={`eventbutton ${isEndEventButtonClicked ? "active" : ""}`} onClick={endButtonClick}>종료된 이벤트</button>
            </Events>
            <EventsWrapper>
        {(isIngEventButtonClicked ? ingEvent : endEvent).map((event) => (
          <EventInfoWrapper key={event.event_name}>
            <EventImg><img className="eventimg" src={event.event_img} alt="" /></EventImg>
            <EventName>{event.event_name}</EventName>
            <EventEnddate>이벤트 종료일: {event.event_enddate}</EventEnddate>
            <EventUrl><a className="eventlink" href={event.event_url} target="_blank" rel="noopener noreferrer">
              이벤트 바로가기</a> 
            </EventUrl>
          </EventInfoWrapper>
        ))}
